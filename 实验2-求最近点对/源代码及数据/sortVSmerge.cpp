...
double Pair(int l,int r)
{
	...
	sort(tmp,tmp+k,cmpy);
	for(int i=0;i<k;i++){
		for(int j=i+1;j<k&&tmp[j].y-tmp[i].y<d;j++){//&&tmp[j].y-tmp[i].y<d
...
int e[10]={1000000,200000,300000,400000,500000};
...
int main()
{
	...
	for(int i=0;i<1;i++){//
		...
		printf("The average time of divide_conquer by sort is %.4fms\n",t2);
		printf("The average time of divide_conquer by merge is %.4fms\n",t3);
		printf("\n");
	}
	//fclose(stdout);//
	return 0;
